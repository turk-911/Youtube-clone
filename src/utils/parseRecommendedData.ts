...
export const parseRecommendedData = async (items: Item[], videoId: string) => {
    try {
       ...
       const { data: {items: videosData},} = await axios.get(`${YOUTUBE_API_URL}/videos?part=contentDetails,statistic&id=${videoIds.join(",")}&key=${API_KEY}`);
       const parsedData: RecommendedVideos[] = [];
       newItems.forEach((item, index) => {
        if (index >= videosData.length) {
            return;
        }
        if (videoId === item?.contentDetails?.upload?.videoId) {
            return;
        }
        parsedData.push({
            videoId: item.contentDetails.upload.videoId,
            videoTitle: item.snippet.title,
            videoThumbnail: item.snippet.thumbnail.medium.url,
